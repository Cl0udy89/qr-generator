...
import { createQRCode, getQRCodes, deleteQRCode, updateQRCode } from '@/lib/api'
import Link from 'next/link'
import { QrCode, ArrowRight, BarChart3, Plus, Link as LinkIcon, Download, Loader2, Target, Trash2, Edit2, Save, X } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete the campaign "${name}"? All analytics will be permanently lost.`)) return;
  ...
  const startEdit = (qr: any) => {
    setEditingId(qr.id)
    setEditCampaignName(qr.campaign_name)
    setEditTargetUrl(qr.target_url)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent, id: string) => {
    ...
    setSavingId(id)
    try {
      await updateQRCode(id, editCampaignName, editTargetUrl)
      setEditingId(null)
      await fetchQRs()
    } catch (err) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 animate-slide-up">
      ...
      <div className="lg:col-span-8 space-y-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {qrs.map((qr, i) => (
              <div key={qr.id} className="glass-panel p-6 rounded-3xl group hover:-translate-y-1 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 hover:border-indigo-500/30 flex flex-col relative" style={{ animationDelay: `${i * 100}ms` }}>

                {deletingId === qr.id && (
                  <div className="absolute inset-0 bg-slate-900/50 rounded-3xl flex items-center justify-center z-10 backdrop-blur-sm">
                    <Loader2 size={32} className="animate-spin text-white" />
                  </div>
                )}

                {editingId === qr.id ? (
                  <form onSubmit={(e) => handleEditSubmit(e, qr.id)} className="flex flex-col h-full space-y-4">
                    <input
                      type="text"
                      required
                      value={editCampaignName}
                      onChange={e => setEditCampaignName(e.target.value)}
                      className="w-full px-4 py-2 bg-slate-100 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-600 rounded-xl outline-none font-extrabold text-xl text-slate-800 dark:text-white"
                      placeholder="Campaign Name"
                    />
                    <div className="flex-1 flex gap-5 items-center">
                      <div className="w-20 h-20 bg-white p-1.5 border-2 border-slate-100 dark:border-slate-700 rounded-2xl shrink-0 shadow-sm opacity-50">
                      ...
                      <div className="flex-1 space-y-1.5 flex flex-col justify-center">
                        <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-600 rounded-lg outline-none text-sm font-semibold text-blue-600 dark:text-blue-400"
                        />
                      </div>
                    </div>
                    <div className="flex gap-2 mt-auto">
                      <button type="button" onClick={cancelEdit} className="flex-1 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-white py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2">
                        <X size={16} /> Cancel
                      </button>
                      <button type="submit" disabled={savingId === qr.id} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2">
                        {savingId === qr.id ? <Loader2 size={16} className="animate-spin" /> : <><Save size={16} /> Save</>}
                      </button>
                    </div>
                  </form>
                ) : (
                  <>
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2.5 py-1 hidden sm:flex items-center gap-1 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700">
                          #{qr.id}
                        </div>
                        <button onClick={() => startEdit(qr)} className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-colors">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => handleDelete(qr.id, qr.campaign_name)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
